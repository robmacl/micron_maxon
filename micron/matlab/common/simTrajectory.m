function simTrajectory(data, data2, timing)
   ...
   % axis([-1100 1100 -1100 1100 -1100 1100]);
    ...
    h(2) = plot3(data2(1,1),data2(1,2),data2(1,3),'g','LineWidth',2);
...
%     
%     h(2) = polar(pol_data2(1,1),pol_data(1,2),'g');
%     set(h(1),'LineWidth',2);
%     set(h(2),'LineWidth',1);
%     
%     
...
%     end
%     animationTimer = timer('ExecutionMode','fixedRate',...  %# Fire at a fixed rate
%                        'Period',0.05,...                %#   every 0.25 seconds
%                        'TasksToExecute',nLen,...          %#   for 40 times and
%                        'TimerFcn',{@timer_fcn_2d h data data2});  %#   run this function
%     start(animationTimer);  %# Start timer, which runs on its own until it ends
%     nPlot = 0;
%     while(nPlot ~= nLen)
%         nPlot = get(animationTimer,'TasksExecuted');    
