function Tasks = loadToIs3(timeStamp, statusFlag, duration)
%Compare with loadToIs2, seperate TOI and ind in each columns
    ...
    for i = 1:nToI
        ...
        if(isempty(remain))
            ...
            target = remain;
            if(length(target)>1), target(1) = []; end
        end
        task = strtok(target, '.');
        ...
        Tasks{i,6} = ind(2);
        Tasks{i,7} = totalTime;
        Tasks{i,8} = convUnixTimeSec(ToI(1));
